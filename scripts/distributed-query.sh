...
def q(n,u,m):
    try:
        s=time.time()
        r=requests.post(u,json={'model':m,'messages':[{'role':'user','content':Q}],'temperature':0.3,'max_tokens':500},timeout=25)
        c=r.json()['choices'][0]['message']['content']
        return {'name':n,'r':re.sub(r'<think>.*?</think>','',c,flags=re.DOTALL).strip()[:800],'t':round(time.time()-s,1),'ok':True}
