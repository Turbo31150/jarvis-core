...
def morning_startup():
    ...
    try:
        mem = MemoryFacade()
        stats = mem.get_stats()
        results["checks"]["db"] = {db: f"{s.get('tables',0)} tables" for db, s in stats.items()}
    except: results["checks"]["db"] = "error"
    ...
    except: results["checks"]["browseros"] = "not available"
...
def end_of_day():
    ...
    runs = c.execute("SELECT COUNT(*) FROM workflow_runs WHERE timestamp LIKE ?", (today+"%",)).fetchone()[0]
    actions = c.execute("SELECT COUNT(*) FROM linkedin_actions WHERE timestamp LIKE ?", (today+"%",)).fetchone()[0]
    conn.close()
...
def incident_triage():
    ...
    if tmp_usage.used / tmp_usage.total > 0.8:
        incidents.append({"type": "disk_full", "path": "/tmp", "usage": f"{tmp_usage.used/tmp_usage.total:.0%}", "severity": "critical"})
    ...
    try:
        ...
        for i, temp in enumerate(r.stdout.strip().split('\n')):
            try:
                if int(temp) > 85:
                    incidents.append({"type": "gpu_hot", "gpu": i, "temp": int(temp), "severity": "critical"})
            except: pass
    except: pass
